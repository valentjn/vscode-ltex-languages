...
class App {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Language language;
        if (Languages.get().stream().anyMatch(x -> x.getShortCodeWithCountryAndVariant().length() > 7)) {
            language = Languages.get().stream().filter(x -> x.getShortCodeWithCountryAndVariant().length() > 7)
